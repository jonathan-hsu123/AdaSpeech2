...
from adaspeech import FeedForwardTransformer
import sys
import torch
...
def evaluate(hp, validloader, model):
    ...
    for valid in validloader:
        x_, input_length_, y_, _, out_length_, ids_, dur_, e_, p_, _ = valid
        
        with torch.no_grad():
            ilens = torch.tensor([x_[-1].shape[0]], dtype=torch.long, device=x_.device)
            _, after_outs, d_outs, e_outs, p_outs, _, _ = model._forward(x_.cuda(), ilens.cuda(), y_.cuda(), out_length_.cuda(), dur_.cuda(), es=e_.cuda(), ps=p_.cuda(), is_inference=False)  # [T, num_mel]
